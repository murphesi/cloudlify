...
class Cloudlify {
    constructor( configuration, profile ) {
		...
		// Configuration is a yaml file - Pre Process and load yaml
		if ( 'string' == typeof configuration && configuration.endsWith( '.yaml' ) ) {
			var config = fs.readFileSync( configuration, 'utf8' );

			// Process yaml with Handlebars to replace with Environment vars from .env
			// Format {{ENV.MY_VAR}} >>>>  .env (file)  MY_VAR = 1234
			var compiledYaml = Handlebars.compile( config )( { ENV: process.env } );

			// Load processed yaml
			this.configuration = yaml.parse( compiledYaml );
		}
